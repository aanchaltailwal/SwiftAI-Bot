...
class ObjectDetectionBot(Bot):
    ...
    def detect_objects(self):
        ...
        res = requests.post(
            f"{YOLO_URL}/predict",
            files={"file": (photo_path, open(photo_path, "rb"), "image/png")},
        )
        ...
        if res.status_code == 200:
            ...
            self.send_text(
                "Failed to perform object detection. Please try again later."
            )
                ...
                for file in files:
                    if file.endswith(".jpg"):
                        ...
                        if res.status_code == 200:
                            ...
                            logger.info(
                                f"Response from detect service with {predictions}"
                            )
                            frame_predictions.append(predictions)

                        else:
                            self.send_text(
                                "Failed to perform object detection on some frames. Please try again later."
                            )
...
if __name__ == "__main__":
    ...
    def handle_draw(message):
        ...
        if image_url:
            ...
            my_bot.send_text_with_quote(
                "Failed to generate the image.", message_id=message.message_id
            )
